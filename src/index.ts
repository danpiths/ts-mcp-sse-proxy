...
class MCPSSEProxy {
  ...
  private parseRequestPath(req: express.Request): { command: string; rest: string } {
    ...
    for (const [name, value] of Object.entries(req.headers)) {
      ...
      if (upperName.startsWith("ENV_")) {
        ...
        if (this.config.blacklistedEnvVars.has(envKey)) {
          this.logger.warn(`Blocked attempt to set blacklisted env var: ${envKey}`, {
            ip: req.ip,
  ...
  private async handleSSEConnection(req: express.Request, res: express.Response): Promise<void> {
    try {
      ...
      // Create MCP server
      const server = new Server({ name: "ts-mcp-sse-proxy", version: "1.0.0" }, { capabilities: {} });
      ...
      childProcess.on("exit", (code, signal) => {
        this.logger.info(`Child process exited: code=${code}, signal=${signal}`, { sessionId });
      ...
      sseTransport.onmessage = (msg: JSONRPCMessage) => {
        ...
        try {
          ...
          this.logger.error(`Failed to write to child process stdin:`, error, { sessionId });
          this.cleanupSession(sessionId);
        }
      };
      ...
      childProcess.stdout.on("data", (chunk: Buffer) => {
        ...
        lines.forEach(line => {
          ...
          try {
            ...
            this.logger.debug(`Child → SSE (session ${sessionId}):`, jsonMsg);
            session.transport.send(jsonMsg);
            session.lastActivity = Date.now();
          } catch {
            this.logger.debug(`Child non-JSON output (session ${sessionId}): ${line}`);
          }
        });
      });
      ...
      childProcess.stderr.on("data", (chunk: Buffer) => {
        this.logger.error(`Child stderr (session ${sessionId}): ${chunk.toString("utf8")}`);
      });
  ...
  private async handleMessagePost(req: express.Request, res: express.Response): Promise<void> {
    try {
      ...
      if (!session?.transport?.handlePostMessage) {
        res.status(503).json({ error: `No active SSE connection for session ${sessionId}` });
        return;
      }
  ...
  private cleanupExpiredSessions(): void {
    ...
    const expiredSessions = Object.entries(this.sessions).filter(
      ([, session]) => now - session.lastActivity > this.config.process.sessionTimeout
    );
  ...
  public start(): void {
    this.app.listen(this.config.env.port, this.config.env.host, () => {
      this.logger.info(`🚀 Server running at http://${this.config.env.host}:${this.config.env.port}`);
      this.logger.info(`📊 Health check: http://localhost:${this.config.env.port}/health`);
      this.logger.info(`🔗 SSE format: http://localhost:${this.config.env.port}/<command>/sse`);
      this.logger.info(`📨 Message format: http://localhost:${this.config.env.port}/<command>/message`);
      this.logger.info(`🛡️ Security: Rate limit ${this.config.rateLimit.requestsPerMinute}/min, Burst ${this.config.rateLimit.burstLimit}`);
